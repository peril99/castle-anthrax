/*
 ...
 HW: https://atlas-scientific.com/kits/wi-fi-pool-kit/
 SW: 
    base: 
...
const String ssid = "yourSID";                                 //The name of the Wi-Fi network you are connecting to
const String pass = "yourWIFIpass";
const char * tagioURI = "/data";
const char * host = "api.tago.io";
const char * tagioToken = "yourtoken";
const int httpsPort =     443;

//Your WiFi network password
//const long myChannelNumber = 1116190;                            //Your Thingspeak channel number
//const char * myWriteAPIKey = "MQ2AJL3HCEBQ5ZMA";                 //Your ThingSpeak Write API Key
//------------------------------------------------------------------

HTTPSRedirect httpsclient(httpsPort);
...
String atlaspH = "this is a long ph";
String atlasTemp = "this is a long temp";
String atlasORP = "this is a long orp";
      ...
      case READ_TEMP_AND_COMPENSATE:
        if (millis() >= next_step_time) {
          ...
          if ((RTD.get_error() == Ezo_board::SUCCESS) && (RTD.get_last_received_reading() > -1000.0)) { //if the temperature reading has been received and it is valid
           ...
           atlasTemp = String(RTD.get_last_received_reading());
           postData("Temp",atlasTemp);
           Serial.print(" Temp: ");
           Serial.print(atlasTemp);
           Serial.print("\n");
      ...
      case READ_RESPONSE:                             //if were in the receiving phase
        if (millis() >= next_step_time) {  //and its time to get the response
          ...
          if (PH.get_error() == Ezo_board::SUCCESS) {                                          //if the PH reading was successful (back in step 1)
            ...
            atlaspH = String(PH.get_last_received_reading());
            postData("pH",atlaspH);
            Serial.print(" pH: ");
            Serial.print(atlaspH);
            Serial.print("\n");
          }
          Serial.print("  ");
          ...
          if (ORP.get_error() == Ezo_board::SUCCESS) {                                          //if the ORP reading was successful (back in step 1)
            //ThingSpeak.setField(2, String(ORP.get_last_received_reading(), 1));
            //assign ORP readings to the second column of thingspeak channel
            atlasORP = String(ORP.get_last_received_reading(), 1);
            postData("ORP",atlasORP);
            Serial.print(" ORP: ");
            Serial.print(atlasORP);
            Serial.print("\n");
          }
          Serial.println();
...
void postData(String tag, String value){
    ...
    String payload =  "{ \"variable\": " + tag + ",  \"value\" : " + value +" }";
    Serial.print("tagioURI");
    Serial.print(tagioURI);
    Serial.print(" host ");
    //Serial.print(host);
    Serial.print("payload ");
    Serial.print(payload);
    //httpsclient.addHeader("Device-Token:", tagioToken);
    httpsclient.POST(tagioURI, host, payload);
    Serial.print("getStatusCode: ");
    Serial.print(httpsclient.getStatusCode());
    Serial.print("getReasonPhrase");
    Serial.print(httpsclient.getReasonPhrase());
    Serial.print(httpsclient.getResponseBody());
}
